...
export default function Home() {
  ...
  const { currentDate, selectedDate, setSelectedDate, setCurrentDate } =
    useCalendarStore();
  const safeDate =
  ...
  const [webEvents, setWebEvents] = useState<Event[]>(() =>
    isWebClient ? readEventsForMonth(year, month) : [],
  );
  const [allWebEvents, setAllWebEvents] = useState<Event[]>(() =>
    isWebClient ? readAllEvents() : [],
  );
  ...
  const selectedDayEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      return (
        ...
        eventDate.toDateString() === safeSelectedDate.toDateString()
      );
    });
  ...
  return (
    <GlassScreen>
      ...
      <MotionView enter="fadeUp" delay={40} style={styles.calendarSection}>
        <LiquidGlassCard size="medium" style={styles.calendarCard} noPadding>
          <MonthView
            ...
            selectedDate={safeSelectedDate ?? undefined}
            events={calendarEvents}
            onDateSelect={setSelectedDate}
            onMonthChange={setCurrentDate}
            onMonthTitlePress={() => router.push("/year_overview")}
            variant="dark"
          />
        </LiquidGlassCard>
      </MotionView>
      ...
      <SearchOverlay
        ...
        suggestions={
          searchSuggestions.length > 0 ? searchSuggestions : undefined
        }
