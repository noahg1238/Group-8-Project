...
import { hasPassword, removePassword } from "../database/auth";
...
export default function Settings() {
  ...
  async function removePasskey() {
    ...
    Alert.alert(
      "Success",
      "Passkey removed successfully! Your app is no longer passkey protected.",
    );
  ...
  function confirmRemoval() {
    Alert.alert(
      "Remove Passkey?",
      "Are you sure you would like to remove your passkey?",
      [
    ...
    <GlassScreen title="Settings" onBack={() => router.back()}>
      ...
      >
        <MotionView enter="fadeUp" duration={280}>
          ...
          >
            <View style={styles.panelInner}>
              <SectionHeader
                title="TIME ZONE"
                delay={staggerMs(0, 80)}
                isFirst
              />
              <SettingsToggleRow
                label="Set Automatically"
                value={timezoneMode === "automatic"}
                onValueChange={handleTimezoneAutomatic}
              />
              <SettingsToggleRow
                label="Set Manually"
                value={timezoneMode === "manual"}
                onValueChange={handleTimezoneManual}
                isLast
              />
              ...
              />
              <SettingsToggleRow
                label="Auto-Decline Invites"
                value={autoDecline}
                onValueChange={(value) =>
                  updateSettings({ autoDecline: value })
                }
                isLast
              />
              ...
              <MotionView enter="fadeUp" delay={staggerMs(3, 80) + 40}>
                <Pressable
                  onPress={() => router.push("/passkey_page")}
                ...
                <Pressable
                  ...
                  style={[
                    styles.removePasskeyButton,
                    (isDeleting || !passwordExists) && styles.disabledText,
                  ]}
                  accessibilityRole="button"
                  disabled={isDeleting || !passwordExists}
                >
                  <Text
                    style={[
                      styles.removePasskeyText,
                      (isDeleting || !passwordExists) && styles.disabledButton,
                    ]}
                  >
                    Remove Passkey
                  </Text>
        ...
        <MotionView enter="fade" delay={320} duration={400}>
          <Text style={styles.footerCopy}>Secure unlock with passkey</Text>
        </MotionView>
      </ScrollView>
    </GlassScreen>
  );
...
const styles = StyleSheet.create({
  ...
  removePasskeyButton: {
    ...
    height: 52,
    borderRadius: 26,
    backgroundColor: "rgba(219, 107, 107, 0.12)",
    marginTop: Spacing.sm,
    marginBottom: Spacing.xs,
  },
  disabledButton: {
