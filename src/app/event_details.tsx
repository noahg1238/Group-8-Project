...
function isOnlineEvent(event: Event): boolean {
	...
	return !event.location || location.includes("online") || location.includes("virtual");
}
...
function EventActionsModal({ visible, onClose, onEdit, onDelete }: { visible: boolean; onClose: () => void; onEdit: () => void; onDelete: () => void }) {
			...
			<Pressable style={styles.actionsBackdrop} onPress={onClose}>
				<View style={styles.actionsSheet}>
					...
					<Pressable style={styles.actionsRow} onPress={onDelete}>
						<Text style={[styles.actionsRowText, styles.actionsDestructive]}>Delete Event</Text>
					</Pressable>
					<View style={styles.actionsDivider} />
...
export default function EventDetailsScreen() {
	...
	const eventDate = useMemo(() => (event ? new Date(event.startDate) : new Date()), [event]);
	...
	return (
		<GlassScreen contentStyle={styles.screenContent} onBack={() => router.back()} hideHeader>
			...
			<ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
				...
				<EventDetailCard event={event} whenLabel={formatEventWhen(event)} calendarLabel={formatCalendarLabel(event)} isOnline={!eventId ? true : isOnlineEvent(event)} onActionPress={showActions} />
				{isError && eventId ? <Text style={styles.fallbackNote}>Showing cached or demo data.</Text> : null}
			</ScrollView>
			...
			<EventActionsModal visible={actionsVisible} onClose={() => setActionsVisible(false)} onEdit={handleEdit} onDelete={() => void handleDelete()} />
		</GlassScreen>
	);
}
